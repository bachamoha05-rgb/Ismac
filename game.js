...
document.getElementById('btn-create-room').onclick = async () => {
    ...
    const indices = [...Array(DB_PHOTOS.length).keys()].sort(() => 0.5 - Math.random()).slice(0, 5);
    await roomRef.set({
        gameState: 'LOBBY', currentRound: 0, photoIndices: indices, roundEndTime: 0,
        players: { [myPlayerId]: { name: myName, score: 0, isHost: true, hasSubmitted: false } }
    });
    listenToRoom();
};
...
function listenToRoom() {
    roomRef.on('value', snap => {
        ...
        if (data.gameState === 'LOBBY') {
            ...
            const c = DB_PHOTOS[data.photoIndices[data.currentRound]].answers;
            ...
            if (isHost && !nextRoundTimer) {
                nextRoundTimer = setTimeout(() => { nextRoundTimer = null; nextRound(data.currentRound); }, 7000);
            }
        }
...
document.getElementById('btn-restart').onclick = () => {
    ...
    roomRef.child('players').once('value', snap => {
        const updates = {};
        snap.forEach(p => { updates[p.key + '/score'] = 0; updates[p.key + '/hasSubmitted'] = false; });
        roomRef.child('players').update(updates);
        const indices = [...Array(DB_PHOTOS.length).keys()].sort(() => 0.5 - Math.random()).slice(0, 5);
        roomRef.update({ gameState: 'LOBBY', currentRound: 0, photoIndices: indices, roundEndTime: 0 });
    });
};
